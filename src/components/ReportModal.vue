<template>
  <div class="fixed inset-0 z-[500] flex items-center justify-center p-3 sm:p-4 bg-slate-900/50 backdrop-blur-xs overflow-y-auto">
    <div class="relative w-full max-w-2xl bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden my-4 select-none">
      ...
      <!-- Form Body -->
      <form @submit.prevent="handleSubmit" class="p-6 space-y-5 max-h-[78vh] overflow-y-auto">
        ...
        <!-- 1. QUALITATIVE AIR QUALITY INTENSITY SELECTION -->
        <div>
          <div class="flex items-center justify-between mb-2">
            <label class="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
              <span>1. Qualitative Air Quality Intensity</span>
              <span class="text-red-500">*</span>
            </label>
            <span class="text-xs font-bold px-2 py-0.5 rounded-full" :class="selectedIntensityBadgeClass">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-5 gap-2">
            ...
            >
              <div>
                ...
                <div class="text-xs font-black tracking-tight leading-tight">{{ item.label }}</div>
                <div class="text-[10px] opacity-75 mt-0.5 leading-snug">{{ item.malayLabel }}</div>
              </div>
        ...
        <div>
          ...
          <div class="mb-2">
            <div class="text-[10px] uppercase font-bold text-slate-400 mb-1">Quick Neighborhood Tags</div>
          ...
          <!-- Manual Location Name & Landmark Input -->
          <div class="flex items-center gap-2">
            <input
              type="text"
              v-model="form.areaName"
              placeholder="Enter neighborhood or landmark (e.g. Taman Tunku, Senadin Phase 3, Permyjaya Mall)"
              required
              class="flex-1 px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-orange-500 focus:bg-white font-medium"
            />
          </div>

          <!-- Precise Coordinates Tagging Display -->
          <div class="mt-1.5 flex items-center justify-between text-[11px] text-slate-400 font-mono">
            <div class="flex items-center gap-3">
        ...
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">
            5. Observable Health Symptoms in Area
          </label>
          <div class="flex flex-wrap gap-1.5">
            <button
              type="button"
        ...
        <!-- 5. QUALITATIVE DESCRIPTION & FASTAPI NLP ANALYSIS -->
        <div>
          <div class="flex items-center justify-between mb-1.5">
            <label class="text-xs font-bold uppercase tracking-wider text-slate-500">
              6. Qualitative Citizen Remarks (English / Bahasa Melayu)
            </label>
            <span class="text-[10px] font-mono text-blue-600 flex items-center gap-1 font-semibold">
              <span class="w-1.5 h-1.5 rounded-full bg-blue-600 animate-pulse"></span>
              FastAPI NLP Live Analyzer
            </span>
          </div>
          <textarea
          ...
          <!-- Live Sentiment & Distress Calculation -->
          <div class="mt-2 p-3 bg-slate-50 rounded-2xl border border-slate-200 text-xs flex items-center justify-between">
...
// Qualitative Air Quality Intensity Levels
